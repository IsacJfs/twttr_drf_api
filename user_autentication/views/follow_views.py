...
def follow_user(request):
    ...
    if user_profile == user_to_follow:
        return Response(
            {"message": "Voce não pode seguir a si mesmo."},
    ...
    return Response(
        {"message": "Usuário seguido com sucesso"}, status=status.HTTP_200_OK
    )
...
def unfollow_user(request):
    ...
    if not user_to_unfollow.followers.filter(id=user_profile.id).exists():
        return Response(
            {"message": "Não pode seguir esse usuário."},
            status=status.HTTP_400_BAD_REQUEST,
        )

    user_to_unfollow.followers.remove(user_profile)
    return Response(
        {"message": "Deixou de seguir o usuário"}, status=status.HTTP_200_OK
    )
